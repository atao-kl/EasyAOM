...
const useAuth = () => {
    ...
    const signup = async (email, password, nom, prenom, nom_entreprise, newslettre, promotions, jconditions, secteuractiviteId) => {
        return axios.post('api/auth/signup',
            { email: email, password: password, nom: nom, prenom: prenom, nom_entreprise : nom_entreprise, newslettre: newslettre, 
                promotions: promotions, jconditions: jconditions, secteuractiviteId: secteuractiviteId })
            .then(res => {
                setToken(res.data.token);
                setClient(res.data.client);
                axios.defaults.headers.common.Authorization = `Bearer ${res.data.token}`;
